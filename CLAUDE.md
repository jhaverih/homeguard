...
Content lives as markdown in `apps/api/legal/*.md` (`customer-terms.md`, `vendor-terms.md`); a version string per document type lives in `apps/api/src/common/constants/tos.ts` (`CURRENT_CUSTOMER_TOS_VERSION`, `CURRENT_VENDOR_TOS_VERSION`, `CURRENT_FACILITATOR_DISCLOSURE_VERSION`); each `User` row stores which version it last accepted (`tosVersion`/`vendorTosVersion`/`facilitatorDisclosureVersion`, each paired with an `*AcceptedAt` timestamp). `GET /legal/versions` exposes the current versions; the mobile app compares its stored version against that via `needsReacceptance()` (`apps/mobile/src/utils/legal.ts`) to decide whether to re-gate a user. `GET /legal/customer-terms.html`/`vendor-terms.html` render the markdown for in-app viewing (`apps/api/src/legal/legal.controller.ts`) — deliberately excluded from the global `/api` prefix (see `main.ts`) since `TERMS_URL`/`VENDOR_TERMS_URL` (`apps/mobile/src/services/api.ts`) are opened directly in the device browser, not called through the authenticated `api` axios instance.
